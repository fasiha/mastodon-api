...
var _streamingApiConnection = _interopRequireDefault(require("./streaming-api-connection"));
...
var Mastodon = /*#__PURE__*/function () {
  ...
  _createClass(Mastodon, [{
    ...
    value: function request(method, path, params, callback) {
      var self = this;
      (0, _assert["default"])(method === 'GET' || method === 'PATCH' || method === 'POST' || method === 'PUT' || method === 'DELETE');
            ...
            Mastodon._doRESTAPIRequest(requestOptions, mastodonOptions, method, function (reqerr, parsedBody, response) {
              ...
              if (self.config.trusted_cert_fingerprints) {
                if (!response.socket.authorized) {
                  ...
                  var merr = _helpers["default"].makeMastodonError("The peer certificate was not signed; ".concat(authErrMsg));
                  _returnErrorToUser(merr);
                  return;
                }
                var _response$socket$getP = response.socket.getPeerCertificate(),
                  fingerprint = _response$socket$getP.fingerprint;
                var trustedFingerprints = self.config.trusted_cert_fingerprints;
                if (!trustedFingerprints.includes(fingerprint)) {
                  var errMsg = _util["default"].format('Certificate untrusted. Trusted fingerprints are: %s. Got fingerprint: %s.', trustedFingerprints.join(','), fingerprint);
    ...
    value: function _buildRequestOptions(method, path, params, callback) {
      ...
      var requestOptions = {
        headers: {
          ...
          Authorization: "Bearer ".concat(this.config.access_token)
        },
        gzip: true,
        encoding: null
      };
      ...
      try {
        ...
        path = _helpers["default"].moveParamsIntoPath(finalParams, path);
      } catch (e) {
      ...
      if (path.match(/^https?:\/\//i)) {
        ...
        requestOptions.url = "".concat(this.apiUrl).concat(path);
      }
      if (finalParams.file) {
    ...
    value: function stream(path, params) {
      ...
      var streamingConnection = new _streamingApiConnection["default"]();
    ...
    value: function _doRESTAPIRequest(requestOptions, mastodonOptions, method, callback) {
      var requestMethod = _request["default"][method.toLowerCase()];
      ...
      request.on('response', function (res) {
        ...
        request.on('end', function () {
          if (body !== '') {
            try {
              ...
              var err = _helpers["default"].makeMastodonError('JSON decode error: Mastodon HTTP response body was not valid JSON');
          ...
          if (_typeof(body) === 'object' && (body.error || body.errors)) {
            ...
            var _err = _helpers["default"].makeMastodonError('Mastodon API Error');
            _err.statusCode = response ? response.statusCode : null;
            _err = _helpers["default"].attachBodyInfoToError(_err, body);
        ...
        request.on('error', function (err) {
          ...
          if (mastodonOptions.retry && _settings.STATUS_CODES_TO_ABORT_ON.includes(err.statusCode)) {
            ...
            err.code = null;
            err.allErrors = [];
            err = _helpers["default"].attachBodyInfoToError(err, body);
            callback(err, body, response);
          }
        });
      });
      ...
      Object.keys(params).forEach(function (key) {
        ...
        if (Array.isArray(value)) {
          value.forEach(function (v) {
            encoded += "".concat(encodeURIComponent(key), "[]=").concat(encodeURIComponent(v), "&");
          });
        } else {
          encoded += "".concat(encodeURIComponent(key), "=").concat(encodeURIComponent(value));
        }
      });
      return encoded;
    }
    ...
    value: function _validateConfigOrThrow(config) {
      if (_typeof(config) !== 'object') {
        throw new TypeError("config must be object, got ".concat(_typeof(config), "."));
      }
      if (typeof config.timeout_ms !== 'undefined' && Number.isNaN(Number(config.timeout_ms))) {
        throw new TypeError("config parameter 'timeout_ms' must be a Number, got ".concat(config.timeout_ms, "."));
      }
      _settings.REQUIRED_KEYS_FOR_AUTH.forEach(function (reqKey) {
        if (!config[reqKey]) {
          throw new Error("Mastodon config must include '".concat(reqKey, "' when using 'user_auth'"));
        }
      });
    }
  }, {
      ...
      return new Promise(function (resolve, reject) {
        _request["default"].post({
          ...
          try {
            ...
            reject(new Error("Error parsing body ".concat(body)));
          }
          resolve(body);
        });
      });
